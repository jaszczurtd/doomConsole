...
static unsigned s_forget_requests;
...
void DoomGamepadInput_RequestForget(void) { ++s_forget_requests; }
...
int main(void) {
  ...
  const uint8_t input_pins[] = {
      DOOM_INPUT_PIN_UP,    DOOM_INPUT_PIN_DOWN,   DOOM_INPUT_PIN_LEFT,
      DOOM_INPUT_PIN_RIGHT, DOOM_INPUT_PIN_FIRE,   DOOM_INPUT_PIN_USE,
      DOOM_INPUT_PIN_MENU,  DOOM_INPUT_PIN_ACCEPT, DOOM_INPUT_PIN_BACK,
  };
  ...
  s_gamepad_actions = DOOM_INPUT_ACTION_UP | DOOM_INPUT_ACTION_DOWN |
                      ...
                      DOOM_INPUT_ACTION_MENU | DOOM_INPUT_ACTION_STRAFE |
  ...
  expect_event(7u, ev_keydown, KEY_RALT);
  expect_event(8u, ev_keydown, ',');
  ...
  expect_event(7u, ev_keyup, KEY_RALT);
  expect_event(8u, ev_keyup, ',');

  clear_events();
  s_gamepad_actions = DOOM_INPUT_ACTION_STRAFE_RIGHT;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keydown, '.');
  clear_events();
  s_gamepad_actions = 0u;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keyup, '.');

  clear_events();
  s_gamepad_actions = DOOM_INPUT_ACTION_ACCEPT;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keydown, KEY_ENTER);
  clear_events();
  s_gamepad_actions = 0u;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keyup, KEY_ENTER);

  clear_events();
  s_gamepad_actions = DOOM_INPUT_ACTION_BACK;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keydown, KEY_BACKSPACE);
  clear_events();
  s_gamepad_actions = 0u;
  I_GetEvent();
  assert(s_event_count == 1u);
  expect_event(0u, ev_keyup, KEY_BACKSPACE);
  ...
  s_now_ms = 12000u;
  s_gamepad_actions = DOOM_INPUT_ACTION_STRAFE_LEFT |
                      DOOM_INPUT_ACTION_STRAFE_RIGHT | DOOM_INPUT_ACTION_BACK;
  I_GetEvent();
  assert(s_forget_requests == 0u);
  assert(s_pairing_requests == 2u);
  s_now_ms = 16999u;
  I_GetEvent();
  assert(s_forget_requests == 0u);
