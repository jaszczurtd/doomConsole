...
#if JASZCZURHAL_PORT
static uint16_t doom_sprite_diag_seen;
...
void DoomRenderSpriteDiag_Get(uint16_t *seen, uint16_t *projected,
                              uint16_t *queued, uint16_t *drawn)
{
    *seen = doom_sprite_diag_seen;
...
{
#if NO_DRAWSEGS && DOOM_SPRITE_OCCLUSION
    extern uint16_t DoomSpriteOcclusion_DebugColumns(void);
    extern uint16_t DoomSpriteOcclusion_DebugClipped(void);
    *columns = DoomSpriteOcclusion_DebugColumns();
    *clipped = DoomSpriteOcclusion_DebugClipped();
#else
    *columns = 0;
    *clipped = 0;
#endif
...
static floor_ceiling_clip_t s_occluder_top[SCREENWIDTH];
static floor_ceiling_clip_t s_occluder_bottom[SCREENWIDTH];
static fixed_t s_occluder_scale[SCREENWIDTH];
static uint16_t s_occluder_columns;
static uint16_t s_occluder_clipped_columns;

static void DoomSpriteOcclusion_Inc(uint16_t *value)
{
    if (*value != 65535u) {
        ++*value;
    }
...
uint16_t DoomSpriteOcclusion_DebugColumns(void) { return s_occluder_columns; }
uint16_t DoomSpriteOcclusion_DebugClipped(void) { return s_occluder_clipped_columns; }

void DoomSpriteOcclusion_Clear(void)
{
    memset(s_occluder_scale, 0, sizeof(s_occluder_scale));
    s_occluder_columns = 0;
    s_occluder_clipped_columns = 0;
}

void DoomSpriteOcclusion_RecordWallColumn(int x, int yl, int yh, fixed_t scale)
{
    if ((unsigned)x >= SCREENWIDTH || yl > yh || scale <= 0) {
        return;
    }
    if (yl < 0) {
        yl = 0;
    ...
    if (scale >= s_occluder_scale[x]) {
        if (s_occluder_scale[x] == 0) {
            DoomSpriteOcclusion_Inc(&s_occluder_columns);
        }
        s_occluder_scale[x] = scale;
        s_occluder_top[x] = (floor_ceiling_clip_t)yl;
        s_occluder_bottom[x] = (floor_ceiling_clip_t)yh;
    }
}
#else
void DoomSpriteOcclusion_Clear(void) {}
...
#endif
    DoomSpriteOcclusion_Clear();
#if JASZCZURHAL_PORT
    doom_sprite_diag_seen = 0;
...
void R_ProjectSprite(mobj_t *thing) {
#if JASZCZURHAL_PORT
    DoomSpriteDiag_Inc(&doom_sprite_diag_seen);
#endif
    fixed_t tr_x;
    fixed_t tr_y;
...
#if JASZCZURHAL_PORT
    DoomSpriteDiag_Inc(&doom_sprite_diag_projected);
#endif
    ...
    vis = R_NewVisSprite();
#if JASZCZURHAL_PORT
    DoomSpriteDiag_Inc(&doom_sprite_diag_queued);
#endif
#endif
...
#else
#if JASZCZURHAL_PORT && DOOM_SPRITE_OCCLUSION
    static floor_ceiling_clip_t clipbot[SCREENWIDTH];
    static floor_ceiling_clip_t cliptop[SCREENWIDTH];

    // Default "no clip" values must follow the FLOOR_CEILING_CLIP_OFFSET
    // convention, otherwise the -1 ceiling sentinel becomes 255 under the
    // 8-bit clip type and silently clips every column to nothing.
    const int default_floor = viewheight + FLOOR_CEILING_CLIP_OFFSET;
    const int default_ceil = FLOOR_CEILING_CLIP_OFFSET - 1;
    // A wall must be meaningfully nearer than the sprite to occlude it; the
    // margin avoids flicker-clipping a sprite that stands right at the wall.
    const fixed_t occlude_scale = spr->scale + (spr->scale >> 4);

    for (x = spr->x1; x <= spr->x2; x++) {
        clipbot[x] = (floor_ceiling_clip_t)default_floor;
        cliptop[x] = (floor_ceiling_clip_t)default_ceil;

        const fixed_t wall_scale = s_occluder_scale[x];
        if (wall_scale == 0 || wall_scale <= occlude_scale) {
            continue;
        }

        const int band_top = s_occluder_top[x];
        const int band_bottom = s_occluder_bottom[x];
        if (band_bottom < band_top) {
            continue;
        }

        // A single (ceiling, floor) window can only keep one contiguous side,
        // so drop the side the opaque wall band reaches.  One-sided walls span
        // the whole opening and reach the floor, so prefer keeping the sprite
        // above the wall; fall back to keeping the larger visible side.
        if (band_bottom >= viewheight - 1) {
            clipbot[x] = (floor_ceiling_clip_t)band_top;
        } else if (band_top <= 0) {
            cliptop[x] = (floor_ceiling_clip_t)band_bottom;
        } else if (band_top >= (viewheight - 1 - band_bottom)) {
            clipbot[x] = (floor_ceiling_clip_t)band_top;
        } else {
            cliptop[x] = (floor_ceiling_clip_t)band_bottom;
        }
        DoomSpriteOcclusion_Inc(&s_occluder_clipped_columns);
    }
    ...
    if (!no_draw_sprites) {
        ...
        if (vissprite_p > vissprites) {
            ...
            for (spr = vsprsortedhead.next;
                ...
                R_DrawSprite(spr);
#if JASZCZURHAL_PORT
                DoomSpriteDiag_Inc(&doom_sprite_diag_drawn);
#endif
            }
        }
#endif
