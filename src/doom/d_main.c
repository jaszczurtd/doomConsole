...
{
#if JASZCZURHAL_PORT
    static int trace_frame_count = 0;
    boolean trace_frame = trace_frame_count < 8;

    if (trace_frame)
    {
        JH_BOOT_TRACE("[frame] %d begin gamestate=%d action=%d visible=%d nodraw=%d\n",
                      trace_frame_count, gamestate, gameaction, screenvisible,
                      nodrawers);
    }
#else
    boolean trace_frame = false;
#endif
#if !DOOM_TINY
    int nowtime;
    int tics;
    ...
    if (trace_frame) JH_BOOT_TRACE("[frame] before I_StartFrame\n");
    I_StartFrame ();
    if (trace_frame) JH_BOOT_TRACE("[frame] after I_StartFrame before TryRunTics\n");

    TryRunTics (); // will run at least one tic
    if (trace_frame) JH_BOOT_TRACE("[frame] after TryRunTics before S_UpdateSounds\n");

    S_UpdateSounds (players[consoleplayer].mo);// move positional sounds
    if (trace_frame) JH_BOOT_TRACE("[frame] after S_UpdateSounds\n");
...
#else
        do {
            if (trace_frame) JH_BOOT_TRACE("[frame] before D_Display wipestate=%d\n", wipestate);
            D_Display();
            if (trace_frame) JH_BOOT_TRACE("[frame] after D_Display wipestate=%d\n", wipestate);
        } while (wipestate);
#endif
    }
...
void D_DoomLoop (void)
{
    JH_BOOT_TRACE("[main] D_DoomLoop begin gamestate=%d action=%d\n",
    ...
    JH_BOOT_TRACE("[main] before I_SetWindowTitle\n");
    I_SetWindowTitle(gamedescription);
    JH_BOOT_TRACE("[main] before I_GraphicsCheckCommandLine\n");
    I_GraphicsCheckCommandLine();
#if !NO_USE_MOUSE
    I_SetGrabMouseCallback(D_GrabMouseCallback);
#endif
    JH_BOOT_TRACE("[main] before I_InitGraphics\n");
    I_InitGraphics();
    JH_BOOT_TRACE("[main] after I_InitGraphics\n");
#if USB_SUPPORT
...
#endif
    JH_BOOT_TRACE("[main] before EnableLoadingDisk\n");
    EnableLoadingDisk();
    JH_BOOT_TRACE("[main] after EnableLoadingDisk before TryRunTics\n");

    TryRunTics();
    JH_BOOT_TRACE("[main] after TryRunTics before V_RestoreBuffer\n");

    V_RestoreBuffer();
    JH_BOOT_TRACE("[main] after V_RestoreBuffer before R_ExecuteSetViewSize\n");
    R_ExecuteSetViewSize();
    JH_BOOT_TRACE("[main] after R_ExecuteSetViewSize before D_StartGameLoop\n");

    D_StartGameLoop();
    JH_BOOT_TRACE("[main] after D_StartGameLoop entering frames\n");
