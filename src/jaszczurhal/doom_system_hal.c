...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
...
#if USE_ZONE_FOR_MALLOC && PICO_ON_DEVICE && JASZCZURHAL_PORT
    extern char __end__;
    ...
    /*
     * The original Pico port turns the whole heap area into Doom zone memory
     * and then routes malloc() into that zone.  The JaszczurHAL port keeps
     * normal malloc() alive for HAL/USB/display code, so the zone must be a
     * real allocation and must leave a small heap reserve.
     */
    heap_start = ((uintptr_t)&__end__ + 3u) & ~(uintptr_t)3u;
    heap_limit = (uintptr_t)&__StackLimit;
    available = heap_limit > heap_start ? (int)(heap_limit - heap_start) : 0;
    allowed = available - JASZCZURHAL_HEAP_RESERVE_BYTES;
    *size = JASZCZURHAL_ZONE_BYTES;
    ...
    *size &= ~3;
    ...
    printf("zone memory request: heap=%p..%p available=%x reserve=%x target=%x\n",
...
#if USE_ZONE_FOR_MALLOC && !JASZCZURHAL_PORT
