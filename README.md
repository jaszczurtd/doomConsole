...
* **doom_tiny_usb** This is a "super tiny" version with additional USB keyboard support. Because of the extra USB 
  ...
  flash).
* **JaszczurHAL port** The current CMake/Arduino-Pico port uses a separate
  raw flash payload for WHD/WHDX data. By default the WHX file is expected at
  `0x10200000`, which requires a board with at least 4 MB of physical flash.
  This keeps the data payload out of the firmware image area.

You can load you WHD/WHX file using [picotool](https://github.com/raspberrypi/picotool). e.g.

```bash
picotool load -v -t bin doom1.whx -o 0x10200000
```
