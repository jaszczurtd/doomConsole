...
SKETCH_DIR="$CMAKE_BUILD_DIR/sketch/rp2040_doom"
...
generated_source_roots = [
    os.path.normpath(os.path.join(sketch_dir, "src")),
    os.path.normpath(os.path.join(arduino_sketch_dir, "src")),
]

for generated_root in generated_source_roots:
    if not os.path.isdir(generated_root):
        continue

    for dirpath, _, filenames in os.walk(generated_root):
        for name in filenames:
            if not name.endswith((".c", ".cc", ".cpp", ".cxx", ".h", ".hh", ".hpp", ".hxx", ".S", ".s")):
                continue

            generated = os.path.normpath(os.path.join(dirpath, name))
            rel = os.path.relpath(generated, generated_root)
            original = os.path.normpath(os.path.join(project_dir, rel))
...
for generated, original in source_map.items():
    if os.path.exists(generated) and os.path.exists(original):
        real_source_map[os.path.realpath(generated)] = original
...
def replace_in_entry(entry, generated, original):
    ...
    if "command" in patched:
        pattern = re.escape(generated) + r"(?![A-Za-z0-9_./-])"
        patched["command"] = re.sub(pattern, original, patched["command"])

    if "arguments" in patched:
        patched["arguments"] = [
            original if arg == generated else arg
